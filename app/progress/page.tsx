          ...
          <div className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800">
            ...
            <div className="grid grid-cols-7 sm:grid-cols-10 gap-2">
              {Array.from({ length: 14 }).map((_, i) => (
                <Skeleton key={`food-skel-${i}`} className="w-full aspect-square rounded-lg" />
              ))}
            </div>
            <div className="flex gap-4 mt-4">
              <div className="flex items-center gap-1"><Skeleton className="w-2.5 h-2.5 rounded" /><Skeleton className="h-3 w-20" /></div>
              <div className="flex items-center gap-1"><Skeleton className="w-2.5 h-2.5 rounded" /><Skeleton className="h-3 w-20" /></div>
              <div className="flex items-center gap-1"><Skeleton className="w-2.5 h-2.5 rounded" /><Skeleton className="h-3 w-12" /></div>
            </div>
          </div>

          {/* 2. EXERCISE COMPLIANCE SKELETON */}
          <div className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800">
            <div className="flex justify-between items-center mb-1">
            ...
            <div className="grid grid-cols-7 sm:grid-cols-10 gap-2">
              {Array.from({ length: 14 }).map((_, i) => (
                <Skeleton key={`ex-skel-${i}`} className="w-full aspect-square rounded-lg" />
              ))}
            </div>
            <div className="flex gap-4 mt-4">
              <div className="flex items-center gap-1"><Skeleton className="w-2.5 h-2.5 rounded" /><Skeleton className="h-3 w-16" /></div>
              <div className="flex items-center gap-1"><Skeleton className="w-2.5 h-2.5 rounded" /><Skeleton className="h-3 w-20" /></div>
              <div className="flex items-center gap-1"><Skeleton className="w-2.5 h-2.5 rounded" /><Skeleton className="h-3 w-16" /></div>
            </div>
          </div>

          {/* Charts Skeletons (Weight, Water, Meal, Sleep, Steps) */}
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800">
              <Skeleton className="h-5 w-36 mb-6" />
              <div className="h-[180px] flex items-end gap-3 px-2">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        ...
        {complianceData.length > 0 && (
          <div className="space-y-6">
            ...
            <div className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800">
              <div className="flex justify-between items-center mb-1">
                <h2 className="font-semibold text-white">🍽️ Food Compliance</h2>
            ...
            <div className="bg-zinc-900 rounded-2xl p-4 border border-zinc-800">
              <div className="flex justify-between items-center mb-1">
                <h2 className="font-semibold text-white">🏃 Exercise Compliance</h2>
                <span className="text-[11px] text-zinc-500">Last {complianceData.length} days</span>
              </div>
              <p className="text-xs text-zinc-500 mb-4">Tap a square to view exercise details</p>
