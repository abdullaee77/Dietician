...
//               setSelectedDay(null)
//             }}
//             className="w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 flex items-center justify-center text-xl active:scale-95 transition"
//           >
//             ‹
//           </button>
...
//             }
//             className="w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-400 flex items-center justify-center text-xl active:scale-95 transition disabled:opacity-30"
//           >
//             ›
//           </button>
//         </div>
...
//             <div key={d} className="text-center text-[11px] text-zinc-600 py-1 font-medium">{d}</div>
//           ))}
//         </div>
...
//                   className={`
//                     aspect-square rounded-xl text-sm font-semibold flex items-center justify-center transition active:scale-90
//                     ${isFuture ? 'text-zinc-800 cursor-default' : ''}
//                     ${!log && !isFuture ? 'text-zinc-700 cursor-default' : ''}
//                     ${log?.completed ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' : ''}
//                     ${log && !log.completed ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : ''}
//                     ${isToday ? 'ring-2 ring-white ring-offset-2 ring-offset-[#0f0f0f]' : ''}
//                     ${isSelected ? 'ring-2 ring-yellow-400 ring-offset-2 ring-offset-[#0f0f0f]' : ''}
//                   `}
//                 >
//                   {day}
//                 </button>
...
//           </span>
//           <span className="flex items-center gap-1.5">
//             <span className="w-3 h-3 rounded bg-zinc-800 border border-zinc-700 inline-block" /> Partial
//           </span>
//           <span className="flex items-center gap-1.5">
//             <span className="w-3 h-3 rounded bg-[#0f0f0f] border border-zinc-800 inline-block" /> No log
//           </span>
//         </div>

//       </div>

//       {/* Day detail modal — background locked */}
//       {selectedDay && (
//         <div
...
//               <button
//                 onClick={() => setSelectedDay(null)}
//                 className="text-zinc-500 text-2xl leading-none w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-800"
//               >
//                 ×
//               </button>
//             </div>
...
//               {/* Meals */}
//               <div>
//                 <p className="text-xs text-zinc-500 uppercase tracking-wide font-medium mb-2">🍽️ Meals</p>
...
//                   ].map(meal => (
//                     <div key={meal.label} className="bg-zinc-800 rounded-xl px-3 py-2.5">
//                       <div className="flex items-center justify-between mb-0.5">
//                         <p className="text-zinc-400 text-xs font-medium">{meal.label}</p>
//                         {meal.time && <p className="text-zinc-500 text-xs">{meal.time}</p>}
//                       </div>
//                       <p className="text-zinc-200 text-sm">
//                         {meal.skipped
//                           ? <span className="text-zinc-500 italic">Skipped</span>
//                           : meal.food || <span className="text-zinc-600">—</span>
//                         }
//                       </p>
//                     </div>
//                   ))}
...
//                         {meal.time && <p className="text-zinc-500 text-xs">{meal.time}</p>}
//                       </div>
//                       <p className="text-zinc-200 text-sm">{meal.food || '—'}</p>
//                     </div>
//                   ))}
//                 </div>
//               </div>
...
//               {/* Sleep */}
//               <div>
//                 <p className="text-xs text-zinc-500 uppercase tracking-wide font-medium mb-2">😴 Sleep</p>
//                 <div className="bg-zinc-800 rounded-xl px-3 py-2.5 flex items-center justify-between">
//                   <p className="text-zinc-200 text-sm font-medium">
//                     {selectedDay.sleep_hours ? `${selectedDay.sleep_hours} hours` : '—'}
//                   </p>
//                   {selectedDay.sleep_time && selectedDay.wake_time && (
...
//                   <p className="text-zinc-400 text-xs mb-1">💧 Water</p>
//                   <p className="text-zinc-200 text-sm font-semibold">{selectedDay.water_glasses ?? 0} glasses</p>
//                 </div>
//                 <div className="bg-zinc-800 rounded-xl px-3 py-3">
//                   <p className="text-zinc-400 text-xs mb-1">👟 Steps</p>
//                   <p className="text-zinc-200 text-sm font-semibold">{selectedDay.steps ?? '—'}</p>
//                 </div>
//               </div>
...
//                     <div className="space-y-2">
//                       {exs.map((ex: any) => (
//                         <div key={ex.id} className="bg-zinc-800 rounded-xl px-3 py-2.5 flex items-center justify-between">
...
//                     <div className="space-y-2">
//                       {foods.map((f: any) => (
//                         <div key={f.id} className="bg-zinc-800 rounded-xl px-3 py-2.5 flex items-center justify-between">
//                           <div className="min-w-0 flex-1">
//                             <p className="text-zinc-200 text-sm truncate">{f.food_name}</p>
//                             <p className="text-zinc-500 text-xs">
//                               {f.food_type === 'skip' ? '🚫 Skip' : '✅ Must eat'}
//                             </p>
//                           </div>
//                           <span className={`ml-3 shrink-0 text-xs font-semibold px-2 py-1 rounded-lg ${
//                             f.complied
...
//                 <div>
//                   <p className="text-xs text-zinc-500 uppercase tracking-wide font-medium mb-2">🍕 Flex Meal</p>
//                   <div className="bg-zinc-800 rounded-xl px-3 py-2.5">
//                     <p className="text-zinc-200 text-sm">{selectedDay.flex_meal}</p>
//                   </div>
//                 </div>
//               )}
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-6 relative z-10">
        ...
        <div className="space-y-3">
          ...
          <div className="bg-zinc-950 border border-zinc-900 rounded-3xl p-5 space-y-4">
            ...
            {/* Exercise Checkin Count */}
            <div className="flex items-center gap-3.5 border-t border-zinc-900 pt-3.5">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Workout Adherence</p>
                <p className="text-sm font-bold text-zinc-100 mt-0.5">
                  {totalExercisesThisMonth > 0 ? `${totalExercisesThisMonth} Total Exercises Logged` : 'No exercises logged this month'}
                </p>
              </div>
            </div>
